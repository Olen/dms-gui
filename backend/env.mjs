...
// Allowlist of binaries the embedded rest-api.py will execute. Defense
// in depth against API-key leakage: even with a valid key, an attacker
// can only invoke the commands dms-gui's backend already uses. The list
// is the audited surface from a sweep of backend/ — extending it
// requires a code change here (and a matching consumer change), which
// keeps the surface visible in code review. Operators can override at
// runtime via DMS_API_ALLOWED_BINS (comma-separated) when they have
// genuinely custom needs.
export const REST_API_DEFAULT_ALLOWED_BINS = [
  // setup.sh subcommands cover most account/alias/quota operations
  'setup',
  // dovecot tooling
  'doveadm',
  'doveconf',
  'dovecot',
  // postfix reload after alias-file edits
  'postfix',
  // process / disk / system probes for the dashboard
  'ps',
  'df',
  'top',
  ...
  'cat',
  'env',
  // alias-file edits (echo append, grep filter + mv replace)
  'echo',
  'mv',
  // rspamd DKIM key install
  'mkdir',
  'cp',
  'chown',
  // pipeline-stage helpers (awk for column projection, base64 for sieve uploads)
  'awk',
  'base64',
  // rspamd EVAL for ratelimit/quarantine inspection
  'redis-cli',
];
...
# Default allowlist mirrors REST_API_DEFAULT_ALLOWED_BINS in backend/env.mjs.
# Operators can override at runtime via DMS_API_ALLOWED_BINS (comma-separated).
DMS_API_DEFAULT_ALLOWED_BINS = {${REST_API_DEFAULT_ALLOWED_BINS.map((b) => `'${b}'`).join(', ')}}
_allowlist_override = os.environ.get('DMS_API_ALLOWED_BINS', '').strip()
if _allowlist_override:
  DMS_API_ALLOWED_BINS = {b.strip() for b in _allowlist_override.split(',') if b.strip()}
else:
  DMS_API_ALLOWED_BINS = DMS_API_DEFAULT_ALLOWED_BINS
        ...
        if not command:
          ...
          try:
            ...
            def run_pipeline(cmd):
              ...
              # Allowlist gate: each pipeline stage's leading binary must
              # be in DMS_API_ALLOWED_BINS. Compare on basename so both
              # 'setup' and '/usr/local/bin/setup' resolve to the same
              # allowlist key. Reject the whole pipeline on first
              # violation — never run a partially-allowed pipeline.
              for stage in stages:
                bin_name = os.path.basename(stage[0])
                if bin_name not in DMS_API_ALLOWED_BINS:
                  logger(f"Rejected: binary '{bin_name}' not in DMS_API_ALLOWED_BINS")
                  return 126, '', f"binary '{bin_name}' not allowed"
